...
export default function Header() {
  ...
  const { data, isLoading } = useSWR(
    user ? "/api/v1/notifikasi/10" : null,
    fetcher,
    {
      ...
      <header className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 shadow-sm z-30">
        ...
        <div className="flex items-center gap-3">
          <button className="relative ">
            ...
            <span className="absolute -top-2 -right-2 bg-secondary rounded-full px-2 py-0.5 text-xs font-bold shadow-md"></span>
          </button>
          <span className="text-sm font-medium bg-secondary mt-4 w-5"></span>
        </div>
      </header>
    );
  return (
    <header className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 shadow-sm z-30">
      ...
      <div className="flex translate-x-10 md:translate-x-0 items-end justify-end md:justify-center md:items-center gap-3">
        ...
        <Link href="/adminGudang/profile" className="flex space-x-2">
          <Image
            src={Person}
            alt="Avatar"
            height={32}
